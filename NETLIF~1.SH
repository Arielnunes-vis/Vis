...
abstract final class ExerciseCatalogSeed {
  ...
  static const List<Exercise> all = [
    ...
    Exercise(
      ...
      difficulty: ExerciseDifficulty.intermediate,
      execution:
          'Tronco inclinado à frente (ou apoiado em banco inclinado), puxe os halteres em direção à cintura mantendo a coluna neutra.',
      breathing: 'Expire ao puxar.',
      commonErrors: ['Arredondar a lombar', 'Usar impulso do corpo'],
      tips: ['Mantenha o tronco estável', 'Aproxime os halteres do quadril'],
      synonyms: ['remada curvada com halteres', 'dumbbell row', 'remada halteres banco inclinado'],
      alternatives: ['barbell_row', 'one_arm_db_row'],
    ),
    Exercise(
      id: 'cable_biceps_curl',
      name: 'Rosca Direta na Polia (Barra W)',
      slug: 'rosca-direta-polia-barra-w',
      primaryMuscle: Muscles.biceps,
      equipment: Equipments.cable,
      category: ExerciseCategory.pull,
      type: ExerciseType.isolation,
      difficulty: ExerciseDifficulty.beginner,
      execution:
          'Segurando a barra W presa à polia baixa, flexione os cotovelos elevando a barra sem mover os ombros.',
      breathing: 'Expire ao subir.',
      commonErrors: ['Balançar o corpo', 'Cotovelos à frente do tronco'],
      tips: ['Cotovelos fixos ao lado do corpo', 'Controle a descida'],
      synonyms: ['rosca direta na polia', 'rosca polia barra w', 'cable curl'],
      alternatives: ['biceps_curl'],
    ),
    Exercise(
      id: 'alternating_db_curl',
      name: 'Rosca Alternada com Halteres',
      slug: 'rosca-alternada-halteres',
      primaryMuscle: Muscles.biceps,
      equipment: Equipments.dumbbell,
      category: ExerciseCategory.pull,
      type: ExerciseType.isolation,
      difficulty: ExerciseDifficulty.beginner,
      execution:
          'Sentado ou em pé, flexione um cotovelo por vez elevando o halter e girando o punho (supinação) até a altura do ombro.',
      breathing: 'Expire ao subir cada braço.',
      commonErrors: ['Balançar o tronco', 'Não girar o punho'],
      tips: ['Alterne os braços em ritmo controlado', 'Cotovelos fixos ao lado do corpo'],
      synonyms: ['rosca alternada', 'rosca alternada com halteres', 'alternating dumbbell curl'],
      alternatives: ['biceps_curl', 'hammer_curl'],
    ),
  ];
...
abstract interface class WorkoutSessionRepository {
  ...
  /// Reenvia as sessões concluídas localmente para a nuvem (Supabase).
  /// Seguro chamar várias vezes (upsert por id).
  Future<void> syncRecentSessions();

  /// Baixa sessões da nuvem e mescla com o histórico local (por id,
  /// sem sobrescrever o que já existe neste aparelho). Usado para
  /// preencher um dispositivo/atalho novo.
...
final class WorkoutSessionRepositoryImpl implements WorkoutSessionRepository {
  WorkoutSessionRepositoryImpl({
    ...
    // Opcional: ausente em testes, presente em produção (syncManagerProvider).
    SyncManager? sync,
  })  : _storage = storage,
  ...
  /// Cache em memória slug -> uuid do `exercise_library` (Supabase),
  /// carregado uma vez por sessão do app.
  Map<String, String>? _exerciseIdCache;
  ...
  Future<WorkoutSummary> finish(WorkoutSession session) async {
    ...
    if (_sync != null && finished.userId.isNotEmpty) {
  ...
  @override
  Future<void> syncRecentSessions() async {
    final sync = _sync;
    final uid = _currentUserId();
    if (sync == null || uid == null || uid.isEmpty) return;
    for (final session in recentSessions(limit: 100)) {
      if (session.userId.isEmpty) continue;
  ...
  Future<void> restoreSessions() async {
    ...
    try {
      sessionRows = await sync.fetchAll('workout_sessions');
      setRows = await sync.fetchAll('exercise_sets');
      slugToId = await _loadExerciseIdCache(sync);
    } catch (_) {
      // Sem conexão ou falha temporária — tenta de novo no próximo boot.
      return;
    }
    if (sessionRows.isEmpty) return;
    final idToSlug = {for (final e in slugToId.entries) e.value: e.key};

    final setsBySession = <String, List<Map<String, dynamic>>>{};
    for (final row in setRows) {
      final sid = row['workout_session_id'] as String?;
      if (sid == null) continue;
      setsBySession.putIfAbsent(sid, () => []).add(row);
    }

    final local = _storage.get<List<dynamic>>(_box, _sessionsKey) ?? const [];
    final byId = <String, Map<String, dynamic>>{
      for (final m in local)
        if (m is Map && m['id'] is String)
          (m['id'] as String): Map<String, dynamic>.from(m),
    };

    for (final row in sessionRows) {
      final id = row['id'] as String?;
      // Não sobrescreve uma sessão que já existe neste aparelho.
      if (id == null || byId.containsKey(id)) continue;
      final session =
          _sessionFromRow(row, setsBySession[id] ?? const [], idToSlug);
      byId[session.id] = session.toMap();
    }

    final merged = byId.values.toList()
      ..sort((a, b) => _rowDate(b).compareTo(_rowDate(a)));
    await _storage.put(_box, _sessionsKey, merged.take(100).toList());
  }

  DateTime _rowDate(Map<String, dynamic> m) {
    final raw = (m['finished_at'] ?? m['started_at']) as String?;
    return raw == null ? DateTime(0) : (DateTime.tryParse(raw) ?? DateTime(0));
  }

  Future<void> _syncSession(WorkoutSession session) async {
    final sync = _sync;
    if (sync == null) return;
    final slugToId = await _loadExerciseIdCache(sync);
    await sync.enqueueAndSync(
      PendingSync(
        id: 'workout_sessions_${session.id}',
    ...
    for (final ex in session.exercises) {
      final exerciseUuid = slugToId[ex.exercise.id];
      for (final set in ex.sets) {
        await sync.enqueueAndSync(
          PendingSync(
            id: 'exercise_sets_${set.id}',
            table: 'exercise_sets',
            operation: SyncOperation.insert,
            payload: _setPayload(session.id, exerciseUuid, set),
            createdAt: DateTime.now(),
          ),
        );
      }
  ...
  /// Busca o catálogo de exercícios (Supabase `exercise_library`) uma
  /// única vez por sessão do app e monta o mapa slug -> uuid — é assim
  /// que cada série sincronizada é ligada ao exercício certo.
  Future<Map<String, String>> _loadExerciseIdCache(SyncManager sync) async {
    if (_exerciseIdCache != null) return _exerciseIdCache!;
    try {
      final rows = await sync.fetchAll('exercise_library');
      _exerciseIdCache = {
        for (final r in rows)
          if (r['slug'] is String && r['id'] is String)
            r['slug'] as String: r['id'] as String,
      };
    } catch (_) {
      _exerciseIdCache = {};
    }
    return _exerciseIdCache!;
  }

  /// Mapeia a sessão local para as colunas de `workout_sessions`
  /// (Supabase). `mood` -> `feeling` e `elapsedSeconds` -> `duration`
  /// porque os nomes das colunas são diferentes dos campos do modelo.
  Map<String, dynamic> _sessionPayload(WorkoutSession s) => {
        'id': s.id,
        'user_id': s.userId,
  ...
  Map<String, dynamic> _setPayload(
    String sessionId,
    String? exerciseUuid,
    WorkoutSetSession set,
  ) =>
      {
        'id': set.id,
  ...
  /// Reconstrói uma [WorkoutSession] a partir das linhas do Supabase —
  /// inverso de [_sessionPayload]/[_setPayload]. Usado para trazer o
  /// histórico de treino para um aparelho/atalho novo.
  WorkoutSession _sessionFromRow(
    ...
    for (final s in setRows) {
      final exUuid = s['exercise_id'] as String?;
      final slug = exUuid != null ? idToSlug[exUuid] : null;
      byExercise.putIfAbsent(slug ?? 'desconhecido', () => []).add(s);
    }

    final sessionId = row['id'] as String;
    final exercises = <WorkoutExerciseSession>[
      for (final entry in byExercise.entries)
        _exerciseSessionFromRows(sessionId, entry.key, entry.value),
    ];
    ...
    return WorkoutSession(
      ...
      finishedAt: row['finished_at'] != null
          ...
          : null,
      exercises: exercises,
      mood: WorkoutMood.fromName(row['feeling'] as String?),
      energy: (row['energy'] as num?)?.toInt(),
      notes: row['notes'] as String?,
      elapsedSeconds: (row['duration'] as num?)?.toInt() ?? 0,
      restSeconds: (row['rest_seconds'] as num?)?.toInt() ?? 0,
      status: SessionStatus.finished,
    );
  }

  WorkoutExerciseSession _exerciseSessionFromRows(
    String sessionId,
    String slug,
    List<Map<String, dynamic>> rows,
  ) {
    final matches = ExerciseCatalogSeed.all.where((e) => e.id == slug);
    final ref = matches.isNotEmpty
        ? _refFromCatalog(matches.first)
        : ExerciseRef(id: slug, name: slug, muscleGroup: '');
    final sorted = [...rows]
    ...
    return WorkoutExerciseSession(
      ...
      sets: [
        for (final s in sorted)
          WorkoutSetSession(
            id: s['id'] as String,
            setNumber: ((s['set_number'] as num?) ?? 1).toInt(),
            type: SetType.fromName(s['set_type'] as String?),
            targetReps: (s['target_reps'] ?? '') as String,
            weight: (s['weight'] as num?)?.toDouble(),
            reps: (s['repetitions'] as num?)?.toInt(),
            rpe: (s['rpe'] as num?)?.toDouble(),
            completed: (s['completed'] as bool?) ?? false,
            restSeconds: (s['rest_seconds'] as num?)?.toInt() ?? 90,
  ...
  ExerciseRef _refFromCatalog(Exercise e) => ExerciseRef(
        id: e.id,
        name: e.name,
        muscleGroup: e.primaryMuscle,
        ...
        commonErrors: e.commonErrors.isEmpty ? null : e.commonErrors.join('; '),
      );
...
abstract interface class BodyProgressRepository {
  ...
  /// Baixa o histórico de peso da nuvem (Supabase) e mescla com o que
  /// já existe salvo neste aparelho (por id, sem duplicar). É o
  /// complemento de [syncWeightHistory]: usado para preencher um
  /// dispositivo/atalho novo que ainda não tem nada local.
  Future<void> restoreWeightHistory();
...
final class BodyProgressRepositoryImpl implements BodyProgressRepository {
  ...
  Future<void> restoreWeightHistory() async {
    ...
    List<Map<String, dynamic>> rows;
    try {
      rows = await sync.fetchAll('weight_history');
    } catch (_) {
      // Sem conexão ou falha temporária — tenta de novo no próximo boot.
      return;
    }
    if (rows.isEmpty) return;
    final local = _store.read(uid, _weight);
    final byId = {
      for (final m in local)
        if (m['id'] is String) m['id'] as String: m,
    };
    for (final row in rows) {
      final record = _weightRecordFromRow(row);
      byId[record.id] = record.toMap();
    }
    await _store.write(uid, _weight, byId.values.toList());
  }

  /// Mapeia uma linha de `weight_history` (Supabase) de volta para o
  /// modelo local — inverso de [_weightPayload].
  WeightRecord _weightRecordFromRow(Map<String, dynamic> row) => WeightRecord(
        id: row['id'] as String,
        userId: row['user_id'] as String? ?? '',
        weight: (row['weight'] as num?)?.toDouble() ?? 0,
        bodyFat: (row['body_fat'] as num?)?.toDouble(),
        muscleMass: (row['muscle_mass'] as num?)?.toDouble(),
        note: row['observation'] as String?,
        source: WeightSource.fromName(row['source'] as String?),
